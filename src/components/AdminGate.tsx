...
export function AdminGate({ children }: { children: React.ReactNode }) {
  ...
  // OPTIONAL: Add specific admin emails here if you only want certain people to have access.
  // Leave the array empty [] if you want to allow any Google account that signs in.
  const ALLOWED_ADMIN_EMAILS = [
    'nasslasu@gmail.com', // Replace or add your official admin email here
  ];

  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        // Check if email whitelist is enabled and if user matches
        if (ALLOWED_ADMIN_EMAILS.length > 0 && !ALLOWED_ADMIN_EMAILS.includes(currentUser.email || '')) {
          ...
          logout(); // Automatically sign out unauthorized users
          setUser(null);
        } else {
          setUser(currentUser);
          setError('');
  ...
  if (loading) {
    return <div className="text-center py-20 text-slate-400">Loading security check...</div>;
  }

  // If NOT logged in, show the Google Sign-in Gate
  if (!user) {
    return (
      <div className="max-w-md mx-auto my-20 p-8 bg-slate-900 border border-white/10 rounded-3xl shadow-2xl backdrop-blur-xl text-center">
  ...
  // If LOGGED IN, render the Admin Panel and a secure Logout button
  return (
    <div>
